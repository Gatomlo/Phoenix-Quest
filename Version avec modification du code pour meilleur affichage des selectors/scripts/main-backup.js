...
$(document).ready(function(){
  ...
  if(localStorage.getItem("scenario") !== null){
    ...
    $.getJSON('scenarios/'+scenario+'/scenario.json',function(data){
      ...
      $('#contentImg').html('<img class="stepImg" src="scenarios/'+scenario+'/img/'+data.mediaRules+'">')
      ...
      // $('#debugData').html('historique: '+history)
  })
})
$(document).on('click','#rules',function(){
    $('#stepName').html("Règles du jeu")
    $('#contentImg').css('display','block')
    $('#contentImg').html('<img class="stepImg" src="scenarios/'+scenario+'/img/'+dataJsonGlobal.mediaRules+'">')
...
function displayTime(actualTimer) {
  var hours = Math.floor(actualTimer / 3600);
  var minutes = Math.floor((actualTimer % 3600) / 60);
  var seconds = actualTimer % 60;
...
function setTimer(){
  ...
  if(dataJsonGlobal.timer){
    if(dataJsonGlobal.countdown != null){
      var actualTimer = dataJsonGlobal.countdown;
      if(localStorage.getItem("actualTimer") !== null){
        actualTimer = Number(localStorage.getItem("actualTimer"));
      }
      displayTime(actualTimer);
      timerInterval = setInterval(function() {
        if(actualTimer === 0){
          ...
          $('.card-footer').css('background-color','#ef6461');
          $('#stepDescription').html('<div>Le temps est malheureusement écoulé.<div>');
          $('#stepName').html('Temsp écoulé');
          $('#contentImg').html('<img class="stepImg" src="medias/timeout.png">');
        } else {
          actualTimer = actualTimer - 1;
          localStorage.setItem("actualTimer", actualTimer);
          displayTime(actualTimer);
        }
      ...
      timerInterval = setInterval(function() {
        ...
        localStorage.setItem("actualTimer", actualTimer);
        displayTime(actualTimer);
        if(currentStep == dataJsonGlobal.lastStepId){
          clearInterval(timerInterval);
        }
...
function addItemOrLocation(itemOrLocationObject){
  //ajout des lieux
  if(itemOrLocationObject.type=="location" && $.inArray(itemOrLocationObject.id,locationAddedHistory)==-1){
    if(itemOrLocationObject.closed && itemOrLocationObject.lockType == null){
        ...
        isClosed = itemOrLocationObject.closed
    }
    else if(itemOrLocationObject.lockType != null){
      ...
      if($.inArray(itemOrLocationObject.id,locationsOpenedHistory)==-1 && itemOrLocationObject.lockType == null){
...
if(dataJsonGlobal.progressBar &&  step.progressBar ){
  var progressBarStep = 100/progressBarStepValue
  if(step.type=="location" && $.inArray(step.id,locationsOpenedHistory)==-1){
    progressBarValue = progressBarValue + progressBarStep
    $('#myBar').css('width', progressBarValue+'%');
  }
  if(step.type!="location" && $.inArray(step.id,history)==-1){
    progressBarValue = progressBarValue + progressBarStep
    $('#myBar').css('width', progressBarValue+'%');
  }
}
  ...
  else{ $('#contentImg').html('<img class="stepImg" src="scenarios/'+scenario+'/img/'+step.media+'">')}
  $('.card-footer').css('background-color','#f3ebde')
    ...
    $('#stepDescription').html("Ce lieu n'est pas encore accessible. Vous devez scanner son Qr code avant de pouvoir l'ouvrir")
      ...
      $.each(steps,function(key,oneLocation){
        if(oneLocation.id==id){
          if($.inArray(oneLocation.code,stepsReplaced ) != -1 ){
            if(jQuery.type(oneLocation.toAssociate)=="array"){
              var positionToCheck = $.inArray(oneLocation.code,stepsReplaced )
              codeToFind = stepsReplacedBy[positionToCheck]
            }
            else{
              codeToFind = oneLocation.replaceBy
            }
            var stepFindedByCode
            $.each(steps,function(key,oneStep){
              if(oneStep.code==codeToFind){
                stepFindedByCode = oneStep
              }
            })
              showStep(stepFindedByCode)
          }
          else{
            showStep(oneLocation)
          }
        }
      })
  }
    ...
    $.each(steps,function(key,oneStep){
      if(oneStep.id==currentStep){
        ...
        if(dataJsonGlobal.hints){
          ...
          if(dataJsonGlobal.hintsCount && dataJsonGlobal.hintsLimit == null){
      ...
      $.each(steps,function(key,oneStep){
        if(oneStep.id == currentStep){
          ...
          if(oneStep.type != 'lock'){
            ...
            if(valueToCheck!=objectId){
              $('.card-footer').css('background-color','#ef6461')
              $('#stepDescription').html('<div">Il n\'est pas possible d\'utiliser cet objet de cette  manière. Essayez-le ailleurs.<div>')
              $('#stepName').html('Action impossible')
              $('#contentImg').html('<img class="stepImg" src="medias/noway.png">')
              return false
            }
        ...
        if(decodeText=="removehint"){
          ...
          $.each(steps,function(key,oneStep){
            ...
            if($.inArray(decodeText,stepsReplaced ) != -1 ){
              $.each(steps,function(key,oneReplaceStep){
                if(oneReplaceStep.code==decodeText){
                  ...
                  localStorage.setItem("locationsOpenedHistory",JSON.stringify(locationsOpenedHistory));
                  if($.inArray(oneReplaceStep.id,locationsOpenedHistory)==-1){
            ...
            if(oneStep.type=='person' && oneStep.code==decodeText){
              ...
              if( oneStep.needBefore != null && ($.inArray(oneStep.needBefore,history ) == -1 || $.inArray(oneStep.needBefore,locationsOpenedHistory ) == -1 )){
                $('.card-footer').css('background-color','#ef6461')
            ...
            if(oneStep.type=='object' && oneStep.code==decodeText){
                if($.inArray(oneStep.id,history) != -1){
                  console.log('show item '+oneStep.id)
                  $('#contentImg').css('display','block')
                  showStep(oneStep)
              ...
              if($.inArray(oneStep.id,history) == -1 && $.inArray(oneStep.needBefore,locationsOpenedHistory) != -1 ){
                  console.log('add item '+oneStep.id)
                $('#contentImg').css('display','block')
                showStep(oneStep)
                removeConnexeSteps(oneStep)
                addConnexeSteps(oneStep)
            ...
            //si c'est un lieu ! il n'est pas possible d'ajouter un lieu dans la liste en le scannant. Ils sont révélés par les personnages, les objets ou les verrous.
...
function inserFuseLock(){
  ...
  $('#contentImg').html('<div id="digit" class="container"><div class="row justify-content-center">'+
                ...
                if(result.pattern==oneStep.lockCode){
                  ...
                  $.each(steps,function(key,newStep){
                    if(newStep.code == oneStep.replaceBy){
                      ...
                      $('#contentImg').css('display','block')
                      $("#actualCode").html("");
                      // $('#stepName').html('<h4>'+newStep.name+'</h4>')
                      // $('#contentImg').html('<img class="stepImg" src="scenarios/mystere_a_la_ferme/img/'+newStep.media+'">')
                      // $('#stepDescription').html(newStep.description)
                      showStep(newStep)
                      currentStep=newStep.id
                      localStorage.setItem("currentStep",currentStep);
                      removeConnexeSteps(newStep)
                    ...
                    $('.card-footer').css('background-color','#ef6461')
                    $('#stepDescription').html('<div>Cela ne semble pas être la bonne combinaison. Veuillez réessayer.<div>')
                    trying = [];
                    $("#actualCode").html("");
                }
                return false
...
function inserSelector(step){
  $('#contentImg').html('<div id="imgHotsop" class="lg-container"><img class=" lg-image stepImg" src="scenarios/'+scenario+'/img/'+step.media+'"></div>')
...
function inserSlidersLock(step){
  var slidersToCreate=step.lockCode
  $(step.lockCode).each(function(index,elem){
      $('#contentImg').append('<div class="row slidersRow"><div class="col slidersName">'+elem[0]+'</div><div class="col-8"><input class="slides" id="slider'+index+'"/></div><div class="col-2 slidersValueNumber" id="value'+index+'"></div></div>')
      $('#slider'+index).slider({
               slide: function (e, value) {
                   $('#value'+index).html('<span class="slidersValue">'+value+'<span>');
                   trying[index]=value
               }
           });
  })
...
$(document).on('click','#passwordSendSoluce',function(e){
    ...
    var steps= dataJsonGlobal.steps
      $.each(steps,function(key,oneStep){
        if(oneStep.id==currentStep){
            if($('#passwordInput').val()==oneStep.lockCode){
              if(oneStep.definitiveReplace){
                stepsReplaced.push(oneStep.code)
                localStorage.setItem("stepsReplaced",JSON.stringify(stepsReplaced));
              }
...
$(document).on('click','#resetGame',function(){
  ...
  caches.keys()
    ...
    .catch((error) => {
     ...
     alert('La fonctionnalité de cache n\'est pas supportée par ce navigateur.');
   }
  localStorage.clear();
  location.reload()
})
