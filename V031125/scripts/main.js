...
document.addEventListener('click', function(event) {
  ...
  if (target.closest('#MobileRules') || target.closest('#rules')) {
    document.getElementById('stepName').innerHTML = "Règles du jeu";

    const contentImg = document.getElementById('contentImg');
    contentImg.style.display = 'block';
    contentImg.innerHTML = `<img class="img-fluid stepImg" src="scenarios/${scenario}/img/${dataJsonGlobal.mediaRules}">`;

    document.getElementById('stepDescription').innerHTML = dataJsonGlobal.rules;
  }
});
//changement de couleurs des boutons dans les modal
...
//choix du scénario
document.addEventListener('click', function(event) {
  if (event.target && event.target.id === 'scenarioButton') {
    let scenarioCode = document.getElementById('scenarioCode').value.toLowerCase().trim();
    console.log('scenarioCode: '+scenarioCode)
    // Afficher l'écran de chargement
    document.getElementById('loadingScreen').style.display = 'block';
    fetch('scenarios/' + scenarioCode + '/scenario.json')
      .then(function(response) {
        if (!response.ok) throw new Error('Not found');
        return response.json();
      })
      .then(function(data) {
        localStorage.setItem('scenario', scenarioCode);
        if (data.sendMail === true) {
          console.log('nécessite une inscription')
          // Récupérer le contenu de la modal d'inscription
          const emailModalContent = document.getElementById('emailModal').querySelector('.modal-content').innerHTML;
          // Remplacer le contenu de la modal scénario par celui de la modal inscription
          document.getElementById('scenarioModal').querySelector('.modal-content').innerHTML = emailModalContent;
          // Afficher la modal scénario mise à jour
          const scenarioModal = new bootstrap.Modal(document.getElementById('scenarioModal'));
          scenarioModal.show();
        } else {
          // Sinon, recharger la page comme avant
          initGame()
        }
      })
      .catch(function() {
        // Masquer l'écran en cas d'erreur
        document.getElementById('loadingScreen').style.display = 'none';
        document.getElementById('errorMessage').innerHTML =
          '</br><div class="alert alert-warning" role="alert"> Ce code de scénario est incorrect!</div>';
...
document.addEventListener('click', function(event) {
  if (event.target && event.target.id === 'emailButton') {
    ...
    const groupMail = document.getElementById('groupEmail').value;
    if (groupName !== "" && emailRegex.test(groupMail)) {
      ...
      document.getElementById('emailErrorMessage').innerHTML =
        '</br><div class="alert alert-warning" role="alert"> Veuillez indiquer un nom de groupe et un Email valide.</div>';
      // Affichage du modal Bootstrap sans jQuery
...
function setTimer() {
  if (!dataJsonGlobal.timer) return;
  timerIsOn = true;
  ...
  const mode = dataJsonGlobal.timerType
  const totalDuration = dataJsonGlobal.timerParam || 0;
  timerInterval = setInterval(function() {
      const now = new Date();
      const elapsedTime = now - startTime;
      let timeToDisplay;
      if (mode === 'endHour') {
        // Calcul de l'heure de fin cible (HH:mm ou HH:mm:ss)
        const [h, m, s = "0"] = dataJsonGlobal.timerParam.split(':');
        const endTime = new Date(now);
        endTime.setHours(parseInt(h, 10));
        endTime.setMinutes(parseInt(m, 10));
        endTime.setSeconds(parseInt(s, 10));
        // Temps restant jusqu'à la cible
        timeToDisplay = elapsedTime;
        if (now >=endTime) {
          clearInterval(timerInterval);
          handleTimeEnd(elapsedTime);
          return;
        }
      } else if (mode === 'countdown') {
        timeToDisplay = totalDuration - elapsedTime;
        if (elapsedTime >= totalDuration) {
          clearInterval(timerInterval);
          handleTimeEnd(totalDuration);
          return;
        }
      } else {
        timeToDisplay = elapsedTime;
      ...
      if (currentStep == dataJsonGlobal.lastStepId && !gameFinished) {
        clearInterval(timerInterval);
        handleGameEnd(elapsedTime);
        return;
      }
      displayTime(timeToDisplay);
    }, 1000);
  }
  function handleTimeEnd(duration) {
    // Désactive tous les éléments avec classes gameInterfaceButton et badge
    document.querySelectorAll('.gameInterfaceButton, .badge').forEach(el => el.disabled = true);
    // Change la couleur de fond des éléments avec classe card-footer
    document.querySelectorAll('.card-footer').forEach(el => el.style.backgroundColor = footerColor);
    document.getElementById('stepDescription').innerHTML = '<div>Le temps est malheureusement écoulé.</div>';
    document.getElementById('stepName').innerHTML = 'Temps écoulé';
    document.getElementById('contentImg').innerHTML = '<img class="img-fluid stepImg" src="medias/timeout.png">';
    const baseFactor = 10000;
    const logTime = Math.log10(duration + 10);
    const progressFactor = progressBarValue / 100;
    const hintPenalty = 1 / (1 + Math.exp(0.5 * (hintsCount - 5)));
    let score = (baseFactor / logTime) * progressFactor * hintPenalty;
    score = Math.min(score, 100000);
    document.getElementById('scoring').innerHTML = '<strong>Votre score est de: ' + Math.ceil(score) + ' points.</strong>';
    sendMail(duration, hintsCount, progressBarValue, Math.ceil(score));
  }
  function handleGameEnd(elapsedTime) {
    // Désactive tous les boutons et badges
    document.querySelectorAll('.gameInterfaceButton, .badge').forEach(el => el.disabled = true);
    const baseFactor = 10000;
    const logTime = Math.log10(elapsedTime + 10);
    const progressFactor = progressBarValue / 100;
    const hintPenalty = 1 / (1 + Math.exp(0.5 * (hintsCount - 5)));
    let score = (baseFactor / logTime) * progressFactor * hintPenalty;
    score = Math.min(score, 100000);
      ...
      if(oneStep.type != 'lock' ){
        ...
        if(valueToCheck==objectId){
          ...
          if(objectStep.autoRemoveObject){
            ...
            actualObjects = actualObjects.filter(item => item !== objectStep)
            localStorage.setItem("actualObjects",JSON.stringify(actualObjects))
          }
          if(oneStep.lockType=='withObject'){
            $('#stepID'+oneStep.id).children('.locationStatus').html(iconOpenedHouse)
      ...
      if(oneStep.type=='location'&& oneStep.code==decodeText){
        if($.inArray(oneStep.id,actualLocations) != -1){
          ...
          if(oneStep.lockType==null){
            ...
            $('#stepID'+oneStep.id).children('.locationStatus').html(iconLock)
          }
          showStep(oneStep)
          removeConnexeSteps(oneStep)
          addConnexeSteps(oneStep)
...
function stopCameraDirectly() {
  // Récupérer l'élément vidéo utilisé par le scanner
  const videoElem = document.querySelector('video'); // ou sélecteur plus spécifique si tu as

  if (videoElem && videoElem.srcObject) {
    videoElem.srcObject.getTracks().forEach(track => {
      track.stop();  // Arrête chaque piste vidéo
    });
    videoElem.srcObject = null; // Libère le flux video de l'élément
    console.log("Caméra arrêtée manuellement via les pistes vidéo");
  } else {
    console.warn("Élément vidéo ou flux introuvable, arrêt caméra impossible.");
  }
}
//fonctions pour insérer les cadenas
...
//   $('#contentImg').html('<div id="imgHotsop" class="responsive-img-wrap lg-container hotspot-container"><img class="lg-image stepImg with-hotspots" src="scenarios/'+scenario+'/img/'+step.media+'"></div>')
...
// }
function inserSelector(step){
  // Créer le conteneur avec l'image
  $('#contentImg').html('<div id="imgHotsop" class="hotspot-container"><img id="hotspotImage" class="stepImg with-hotspots" src="scenarios/'+scenario+'/img/'+step.media+'"></div>');

  var buttonsToAdd = step.lockCode;

  // Attendre que l'image soit complètement chargée
  $('#hotspotImage').on('load', function() {
    const img = this;
    const imgWidth = img.naturalWidth;  // Largeur réelle de l'image
    const imgHeight = img.naturalHeight; // Hauteur réelle de l'image

    $(buttonsToAdd).each(function(index, thisButton){
      // thisButton[0] contient le style inline avec les pixels
      // Extraire les valeurs top et left en pixels
      const styleString = thisButton[0];
      ...
      if (topMatch && leftMatch) {
        ...
        console.error('Format de style invalide:', styleString);
      }
    });
  });

  // Si l'image est déjà dans le cache, déclencher load manuellement
  if ($('#hotspotImage')[0].complete) {
    $('#hotspotImage').trigger('load');
  }
}
...
function inserSonoMeter(step){
  ...
  $('#contentImg').html( '<div class="row"><div id="qrCodeReaderForSonometer"></div></div></br><div class="row align-items-start"><div class="col" id="listOfSymbols">'+symbolsText
...
function inserSlidersLock(step){
  ...
  $(step.lockCode).each(function(index, elem) {
    ...
    $('#contentImg').append('<div class="row slidersRow align-items-center mb-1">' +
    '<label for="slider' + index + '" class="col-auto form-label">' + elem[0] + '</label>' +
    '<div class="col-auto result" id="result' + index + '">0</div>' +
    '</div>'+'<div class="row mb-3">' +'<input value="0" min="0" max="200" type="range" data-index="' + index + '" id="slider' + index + '" class="form-range multi-range col">' + '</div>');
  });
...
$(document).on('click','#sendSoluce',function(e){
  ...
  $.each(steps,function(key,oneStep){
    ...
    if(tryingCode==responseCode){
      ...
      $('#myToast').toast('show');
      if(oneStep.lockType!="fuse" && oneStep.lockType!=("sliders")){
        trying = [];
      }
...
function ifCodeSivalide(oneStep){
  locationsOpenedHistory.push(oneStep.id)
  localStorage.setItem("locationsOpenedHistory",JSON.stringify(history));
  history.push(oneStep.id)
  localStorage.setItem("storageHistory",JSON.stringify(history));
